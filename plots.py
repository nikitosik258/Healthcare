...
def plot_feature_importance(model, feature_names, top_n=None, figsize=(10, 6),
    ...
    Parameters:
    - model: Trained model (DecisionTree, RandomForest, LogisticRegression, LinearRegression, etc.)
    ...
    if model_type == 'tree':
        ...
        # Для линейных моделей берём модуль коэффициентов
        coef = np.asarray(model.coef_)
        if coef.ndim == 1:
            # обычная регрессия или бинарная логрегрессия
            importances = np.abs(coef)
        else:
            # мультикласс — усредняем по классам
...
def compare_metrics_heatmap(df1, df2, df1_name='DF1', df2_name='DF2',
    ...
    """
    # Убираем MSE, если он есть
    cols_to_use = [c for c in df1.columns if c != "MSE" and c in df2.columns]
    df1_ = df1[cols_to_use]
    df2_ = df2[cols_to_use]

    # Calculate delta (difference) between DataFrames
    delta = df2_ - df1_

    # Create a custom red-white-green colormap
    colors = ["#00b975", "#ffffff", "#ff2700"]  # Red -> White -> Green
...
def plot_regression_results(metrics, model_name="Model"):
    """
    Визуализация результатов регрессии по агрегированным метрикам.
    ...
    """
    ordered_keys = ["MAE", "RMSE", "R2"]
    plot_metrics = {k: metrics[k] for k in ordered_keys if k in metrics}
...
def print_regression_report(metrics, model_name="Model"):
    """
    Печать таблички с метриками регрессии.

    Ожидает словарь, как в calculate_regression_metrics.
    """
    ...
    for k in ordered_keys:
        if k in metrics:
            rows.append((k, metrics[k]))

    df_report = pd.DataFrame(rows, columns=["Metric", "Value"])
